...
The agropass domain layer on top of [`@symblon/core`](../../README.md) — the agriculture / fruit-traceability registry's chain semantics. It applies the engine's domain-neutral primitives with agropass meaning; the engine validates structure and tamper-binding, this layer owns the semantics.

Design: [`docs/superpowers/specs/2026-06-26-agropass-trust-model-design.md`](../../docs/superpowers/specs/2026-06-26-agropass-trust-model-design.md).

## What's here (v1)

Two chain roles, both ordinary `@symblon/core` chains distinguished by subject scheme:

- **Lot chains** (`agropass.lot`) — the goods, custody baton-passed grower → wholesaler → retailer via the engine's `custody_change`. One continuous track-and-trace timeline; the existing engine, unchanged.
...
import {
  ...
  COUNTER_CLAIM,                        // agropass dispute event type
...
`verifyDispute` wraps the engine's `verifyReference` and adds the agropass semantics the engine deliberately omits: the `agropass.party` / `agropass.lot` scheme roles and the `disputes` relationship (the engine carries `rel` but never interprets it).

## Test / typecheck

```bash
npm test -w @symblon/agropass        # the runnable end-to-end dispute scenario lives in the tests
npm run typecheck -w @symblon/agropass
```
